...
export function dbOrderToDashboardOrder(order) {
  ...
  const changeHistory = order.order_events ?? [];
  ...
  const needsOperationalReview =
    !media.review_confirmed &&
    (order.needs_review ||
      Boolean(media.has_audio) ||
      Boolean(media.has_images) ||
      Boolean(media.requires_transcription) ||
      Boolean(media.requires_image_reading) ||
  ...
  return {
    ...
    customer: {
      ...
      needs_review: needsOperationalReview
    },
    seller: {
    ...
    items: normalizeDashboardItems(order, items),
    ...
    status: uiStatus,
    dbIds: [order.id],
    carrierName: media.carrier_name ?? null,
    reviewConfirmed: Boolean(media.review_confirmed),
    reviewConfirmedAt: media.review_confirmed_at ?? null,
    reviewConfirmedBy: media.review_confirmed_by ?? null,
    ...
    changeHistory: normalizeOrderEvents(changeHistory)
  };
}
...
export function liveOrderToDashboardOrder(order) {
  ...
  return {
    ...
    originalText: order.original_text ?? "",
    attachmentFilenames: media.filenames ?? [],
    changeHistory: normalizeOrderEvents(order.events ?? [])
  };
}
...
export function groupOpenDashboardOrders(orders) {
  ...
  for (const order of orders) {
    ...
    existing.changeHistory = [...(existing.changeHistory ?? []), ...(order.changeHistory ?? [])].sort(
    ...
    existing.carrierName = existing.carrierName ?? order.carrierName ?? null;
    existing.reviewConfirmed = Boolean(existing.reviewConfirmed && order.reviewConfirmed);
    existing.reviewConfirmedAt = existing.reviewConfirmedAt ?? order.reviewConfirmedAt ?? null;
    existing.reviewConfirmedBy = existing.reviewConfirmedBy ?? order.reviewConfirmedBy ?? null;
...
function normalizeOrderEvents(events = []) {
  return events
    .map((event) => ({
      id: event.id,
      type: event.event_type,
      label: eventLabel(event.event_type, event.payload),
...
function eventLabel(type, payload = {}) {
  const actor = payload.actor ? `${payload.actor} - ` : "";
  if (type === "status_changed") return `${actor}Estado: ${payload.status ?? "actualizado"}`;
  if (type === "carrier_changed") return `${actor}Transportista: ${payload.carrierName ?? "sin asignar"}`;
  if (type === "order_corrected") return `${actor}Pedido corregido manualmente`;
  if (type === "review_confirmed") return `${actor}Revision confirmada`;
  if (type === "item_updated") return `${actor}Producto actualizado: ${payload.productNormalized ?? "producto"}`;
  return type ?? "Cambio";
}
...
function normalizeDashboardItems(order, items) {
  ...
  return splitHistoricalIbcMixedItem(order, mapped);
}

function splitHistoricalIbcMixedItem(order, items) {
  const text = order.original_text ?? "";
  const hasIbcChemical = /(?:^|\s)(?:un|una|1)?\s*ibc\s+(?:de\s+)?(?<chemical>cloro|hipoclorito|nitrico|acido nitrico|peracetico|detergente)\b/i.exec(text);
  if (!hasIbcChemical) return items;

  return items.flatMap((item) => {
    const product = `${item.product_original} ${item.product_normalized}`.toLowerCase();
    if (!product.includes("cloro") || !product.includes("guante") || item.quantity === null) return [item];

    return [
      {
        ...item,
        id: `${item.id}_ibc`,
        product_original: hasIbcChemical.groups.chemical,
        product_normalized: canonicalProductName(hasIbcChemical.groups.chemical),
        quantity: 1,
        unit: "ibc"
      },
      {
        ...item,
        product_original: "guantes negros TXL",
        product_normalized: "guantes negros txl"
      }
    ];
  });
}
