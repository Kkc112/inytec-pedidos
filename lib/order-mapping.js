...
function eventLabel(type, payload = {}) {
  if (type === "status_changed") return `Estado: ${payload.status ?? "actualizado"}`;
  if (type === "carrier_changed") return `Transportista: ${payload.carrierName ?? "sin asignar"}`;
  if (type === "order_corrected") return "Pedido corregido manualmente";
  if (type === "item_updated") return `Producto actualizado: ${payload.productNormalized ?? "producto"}`;
  return type ?? "Cambio";
}
