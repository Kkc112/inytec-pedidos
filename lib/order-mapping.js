...
export function dbOrderToDashboardOrder(order) {
  ...
  return {
    ...
    items: items.map((item) => ({
      id: item.id,
      product_original: stripKnownCustomerAlias(item.product_text),
      product_normalized: canonicalProductName(stripKnownCustomerAlias(item.product_normalized ?? item.product_text)),
...
export function liveOrderToDashboardOrder(order) {
  ...
  return {
    ...
    items: (order.items ?? []).map((item) => ({
      product_original: stripKnownCustomerAlias(item.productText),
      product_normalized: canonicalProductName(stripKnownCustomerAlias(item.productNormalized ?? item.productText)),
      quantity: item.quantity === null ? null : Number(item.quantity),
      unit: item.unit,
